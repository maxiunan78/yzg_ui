...
class TestRegister:
    ...
    def test_register(self, start, case):
        ...
        with allure.step(U'检查是否注册'):
            assert result == None ,log.info("已经注册了！不再继续执行注册用例")
        ...
        with allure.step(u'选择站点'):
            ...
            sql = "SELECT station_name FROM erp_station.station WHERE STATION_ID={}".format(yaml_handle.param_value('stationId'))
            result = DB_sql().select_db(sql=sql)
