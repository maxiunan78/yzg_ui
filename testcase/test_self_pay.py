...
class TestSelfPay:
    ...
    def test_order_pay(self, start):
        with allure.step(U'确认订单信息'):
            ...
            TestSelfPay.tmp.update({U'订单ID': custom.get_params(start.get_cur_url())['selfPayOrderId']})
            assert TestSelfPay.tmp[U'油品'] == order_oil, log.error(U'油品错误')
            assert TestSelfPay.tmp[U'订单金额'] == order_amt, log.error(U'金额错误')
        with allure.step(U'确认优惠金额'):  # UI上的数据核对
            ...
            pay_count = (order_amt - discount) > 0.00 and (order_amt - discount) or 0.00
            assert pay_amt == pay_count, log.error(U'支付金额错误')
        # 预计这笔订单流水
        # Preconditions
